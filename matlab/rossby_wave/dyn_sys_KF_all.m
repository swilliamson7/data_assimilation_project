...
for j = T:-1:2
    ...
    temp = (A * uncertainty{j-1} * A' + Gamma * Q * Gamma')^(-1);
    L = uncertainty{j-1} * A' * temp;
    M = Q * Gamma' * temp;

    all_states_RTS(:, j) = all_states_KF(:, j) + ...
                L * (all_states_RTS(:, j) - all_states(:, j));
...
nexttile;
plot(0:dt:T*dt, energy, 0:dt:T*dt, energy_KF, '--', 0:dt:T*dt, energy_RTS, '-.', 'linewidth', 1.5)
title('True Energy')
xline(data_steps*dt)
